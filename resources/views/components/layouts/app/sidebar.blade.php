<mijnui:sidebar x-data variant="double">
    ...
    <mijnui:sidebar.double>
        ...
        <mijnui:button mijnui-sidebar-parent="dashboard" title="Dashboard" size="icon-sm" ghost>
        ...
        <!-- User Icon -->
        <mijnui:button mijnui-sidebar-parent="user" title="User" size="icon-sm" ghost>
        ...
        <!-- Device Icon -->
        <mijnui:button mijnui-sidebar-parent="device" title="Device" size="icon-sm" ghost>
        ...
        <!-- Setting Icon -->
        <mijnui:button mijnui-sidebar-parent="setting" title="Setting" size="icon-sm" ghost>
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="size-4">
                <path fill-rule="evenodd"
    ...
    <mijnui:sidebar.double.content mijnui-sidebar-child="user" title="User Management">
        <mijnui:list class="flex h-full w-full flex-col items-center gap-2 px-4 py-4">
            ...
            <mijnui:list.item href="{{ route('role-permissions.index') }}"
                active="{{ request()->routeIs('role-permissions.index') }}" wire:navigate>Role List</mijnui:list.item>
        </mijnui:list>
    </mijnui:sidebar.double.content>

    <mijnui:sidebar.double.content mijnui-sidebar-child="device" title="Device Management">
        <mijnui:list class="flex h-full w-full flex-col items-center gap-2 px-4 py-4">
            <mijnui:list.item href="{{ route('devices.index') }}" active="{{ request()->routeIs('devices.index') }}"
                wire:navigate>Devices List</mijnui:list.item>
            <mijnui:list.item href="{{ route('protocols.index') }}"
                active="{{ request()->routeIs('protocols.index') }}" wire:navigate>Protocol List</mijnui:list.item>
            <mijnui:list.item href="{{ route('sensors.index') }}" active="{{ request()->routeIs('sensors.index') }}"
                wire:navigate>Sensors</mijnui:list.item>
        </mijnui:list>
    </mijnui:sidebar.double.content>
